<?php
if (!defined('ABSPATH')) exit;

function anuario_render_comments() {
  global $wpdb;
  $table_alumni    = $wpdb->prefix . 'anuario_alumni';
  $table_comments  = $wpdb->prefix . 'anuario_comentarios';

  /* =========================================================
     CREAR COMENTARIO
  ========================================================= */
  $notice = '';
  if (isset($_POST['create_comment'])) {
    check_admin_referer('anuario_create_comment');

    if (empty($_POST['confirm_submit'])) {
      $notice = ['error', 'Debes confirmar antes de guardar el comentario.'];
    } else {
      $alumni_id  = intval($_POST['alumni_id']);
      $comentario = sanitize_textarea_field($_POST['comentario']);

      if (!$alumni_id || !$comentario) {
        $notice = ['error', 'El alumni y el comentario son obligatorios.'];
      } else {
        $wpdb->insert($table_comments, [
          'alumni_id'  => $alumni_id,
          'comentario' => $comentario,
        ]);
        $notice = ['success', 'Comentario guardado correctamente.'];
      }
    }
  }

  /* =========================================================
     FILTROS Y LISTADO
  ========================================================= */
  $per_page       = 20;
  $paged          = max(1, intval($_GET['paged'] ?? 1));
  $offset         = ($paged - 1) * $per_page;
  $filter_alumni  = intval($_GET['alumni_id'] ?? $_GET['id'] ?? 0);

  $where = "WHERE 1=1";
  if ($filter_alumni) {
    $where .= $wpdb->prepare(" AND c.alumni_id = %d", $filter_alumni);
  }

  $total = $wpdb->get_var(
    "SELECT COUNT(*) FROM $table_comments c $where"
  );
  $comments = $wpdb->get_results(
    "SELECT c.*, a.nombre, a.rut
     FROM $table_comments c
     JOIN $table_alumni a ON c.alumni_id = a.id
     $where
     ORDER BY c.created_at DESC
     LIMIT $per_page OFFSET $offset"
  );
  $pages = ceil($total / $per_page);

  // Todos los alumni para el selector
  $all_alumni = $wpdb->get_results(
    "SELECT id, nombre, rut FROM $table_alumni ORDER BY nombre ASC",
    ARRAY_A
  );

  $preselected_id = $filter_alumni;

  // Nombre del alumni filtrado (para el título contextual)
  $alumni_filtrado = null;
  if ($filter_alumni) {
    $alumni_filtrado = $wpdb->get_row($wpdb->prepare(
      "SELECT nombre FROM $table_alumni WHERE id = %d",
      $filter_alumni
    ));
  }
  ?>

  <div class="wrap">
    <h1 class="wp-heading-inline">
      <?php echo $alumni_filtrado
        ? 'Comentarios de ' . esc_html($alumni_filtrado->nombre)
        : 'Comentarios de Alumni'; ?>
    </h1>

    <a href="<?php echo esc_url(admin_url('admin.php?page=anuario-alumni')); ?>"
       class="button button-secondary" style="margin-left:10px;">
      ← Volver al listado
    </a>

    <?php if ($notice): ?>
      <div class="notice notice-<?php echo $notice[0]; ?> is-dismissible" style="margin-top:15px;">
        <p><?php echo esc_html($notice[1]); ?></p>
      </div>
    <?php endif; ?>

    <!-- =====================================================
         FORMULARIO CREAR COMENTARIO
    ====================================================== -->
    <div style="background:#fff;border:1px solid #ccd0d4;padding:20px;margin-top:20px;max-width:700px;">
      <h2 style="margin-top:0;">Agregar comentario</h2>

      <form method="post">
        <?php wp_nonce_field('anuario_create_comment'); ?>

        <table class="form-table">
          <tr>
            <th>Buscar alumni</th>
            <td>
              <input type="search" id="alumni-search"
                     placeholder="Buscar por nombre o RUT..."
                     class="regular-text"
                     autocomplete="off">
            </td>
          </tr>
          <tr>
            <th>Alumni *</th>
            <td>
              <select name="alumni_id" id="alumni_id_select" required class="regular-text">
                <option value="">— Seleccionar alumni —</option>
                <?php foreach ($all_alumni as $a): ?>
                  <option value="<?php echo $a['id']; ?>"
                          data-nombre="<?php echo esc_attr(strtolower($a['nombre'])); ?>"
                          data-rut="<?php echo esc_attr($a['rut'] ?? ''); ?>"
                          <?php selected($preselected_id, $a['id']); ?>>
                    <?php echo esc_html($a['nombre']); ?>
                    <?php if ($a['rut']): ?> — <?php echo esc_html($a['rut']); ?><?php endif; ?>
                  </option>
                <?php endforeach; ?>
              </select>
            </td>
          </tr>
          <tr>
            <th>Comentario *</th>
            <td>
              <textarea name="comentario" rows="4" class="large-text" required
                        placeholder="Escribe el comentario aquí..."></textarea>
            </td>
          </tr>
          <tr>
            <th>Confirmación</th>
            <td>
              <label>
                <input type="checkbox" name="confirm_submit" required>
                Confirmo que deseo guardar este comentario
              </label>
            </td>
          </tr>
        </table>

        <?php submit_button('Guardar comentario', 'primary', 'create_comment'); ?>
      </form>
    </div>

    <!-- =====================================================
         FILTRO + ACCIONES MASIVAS
    ====================================================== -->
    <div style="display:flex;align-items:center;gap:10px;margin-top:25px;flex-wrap:wrap;">
      <form method="get" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        <input type="hidden" name="page" value="anuario-alumni-comments">

        <select name="alumni_id" class="regular-text">
          <option value="">Todos los alumni</option>
          <?php foreach ($all_alumni as $a): ?>
            <option value="<?php echo $a['id']; ?>" <?php selected($filter_alumni, $a['id']); ?>>
              <?php echo esc_html($a['nombre']); ?>
              <?php if ($a['rut']): ?> — <?php echo esc_html($a['rut']); ?><?php endif; ?>
            </option>
          <?php endforeach; ?>
        </select>

        <button class="button">Filtrar</button>

        <?php if ($filter_alumni): ?>
          <a href="<?php echo admin_url('admin.php?page=anuario-alumni-comments'); ?>" class="button">
            Limpiar filtro
          </a>
        <?php endif; ?>
      </form>

      <button id="bulk-delete-comments-btn" class="button" style="color:#b91c1c;">
        Eliminar seleccionados
      </button>
    </div>

    <!-- =====================================================
         TABLA DE COMENTARIOS
    ====================================================== -->
    <form method="post" id="bulk-delete-comments-form">
      <?php wp_nonce_field('bulk_delete_comments'); ?>

      <table class="wp-list-table widefat striped" style="margin-top:15px;">
        <thead>
          <tr>
            <th style="width:30px;"><input type="checkbox" id="select-all-comments"></th>
            <th>Alumni</th>
            <th>RUT</th>
            <th>Comentario</th>
            <th>Fecha</th>
            <th>Acciones</th>
          </tr>
        </thead>
        <tbody>
          <?php if ($comments): foreach ($comments as $c): ?>
            <tr>
              <td><input type="checkbox" name="comment_ids[]" value="<?php echo $c->id; ?>"></td>
              <td><?php echo esc_html($c->nombre); ?></td>
              <td><?php echo esc_html($c->rut ?? '—'); ?></td>
              <td><?php echo esc_html($c->comentario); ?></td>
              <td><?php echo esc_html($c->created_at); ?></td>
              <td>
                <a href="<?php echo wp_nonce_url(
                  admin_url('admin.php?page=anuario-alumni-comments&delete_comment=' . $c->id),
                  'delete_comment'
                ); ?>"
                   onclick="return confirm('¿Eliminar este comentario? Esta acción no se puede deshacer.')">
                  Eliminar
                </a>
              </td>
            </tr>
          <?php endforeach; else: ?>
            <tr><td colspan="6">No hay comentarios registrados.</td></tr>
          <?php endif; ?>
        </tbody>
      </table>
    </form>

    <?php if ($pages > 1): ?>
      <div class="tablenav-pages" style="margin-top:10px;">
        <?php for ($p = 1; $p <= $pages; $p++): ?>
          <a class="button <?php echo $p === $paged ? 'button-primary' : ''; ?>"
             href="<?php echo add_query_arg('paged', $p); ?>">
            <?php echo $p; ?>
          </a>
        <?php endfor; ?>
      </div>
    <?php endif; ?>
  </div>

  <!-- =====================================================
       MODAL CONFIRMACIÓN ELIMINACIÓN MASIVA
  ====================================================== -->
  <div id="bulk-delete-modal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9999;align-items:center;justify-content:center;">
    <div style="background:#fff;border-radius:4px;padding:30px;max-width:440px;width:90%;box-shadow:0 4px 20px rgba(0,0,0,.2);">
      <h2 style="margin-top:0;">Confirmar eliminación</h2>
      <p>Estás a punto de eliminar <strong id="modal-count"></strong>. Esta acción no se puede deshacer.</p>
      <p style="color:#b91c1c;font-weight:600;">¿Deseas continuar?</p>
      <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:20px;">
        <button id="modal-cancel" class="button">Cancelar</button>
        <button id="modal-confirm" class="button button-primary" style="background:#b91c1c;border-color:#b91c1c;">
          Sí, eliminar
        </button>
      </div>
    </div>
  </div>

  <script>
  document.getElementById('alumni-search').addEventListener('input', function () {
    const term = this.value.toLowerCase();
    const select = document.getElementById('alumni_id_select');

    Array.from(select.options).forEach(opt => {
      if (!opt.value) return;
      const nombre = opt.dataset.nombre || '';
      const rut    = opt.dataset.rut    || '';
      opt.hidden = term.length > 0 && !nombre.includes(term) && !rut.includes(term);
    });
  });

  document.getElementById('select-all-comments').addEventListener('change', e => {
    document.querySelectorAll('input[name="comment_ids[]"]').forEach(cb => cb.checked = e.target.checked);
  });

  const modal      = document.getElementById('bulk-delete-modal');
  const modalCount = document.getElementById('modal-count');

  document.getElementById('bulk-delete-comments-btn').addEventListener('click', () => {
    const selected = Array.from(document.querySelectorAll('input[name="comment_ids[]"]:checked'));
    if (!selected.length) {
      alert('Debes seleccionar al menos un comentario.');
      return;
    }
    const n = selected.length;
    modalCount.textContent = n === 1 ? '1 comentario' : `${n} comentarios`;
    modal.style.display = 'flex';
  });

  document.getElementById('modal-cancel').addEventListener('click', () => {
    modal.style.display = 'none';
  });

  document.getElementById('modal-confirm').addEventListener('click', () => {
    const form = document.getElementById('bulk-delete-comments-form');
    form.action = "<?php echo admin_url('admin.php?page=anuario-alumni-comments'); ?>";
    form.submit();
  });

  modal.addEventListener('click', e => {
    if (e.target === modal) modal.style.display = 'none';
  });
  </script>
  <?php
}
